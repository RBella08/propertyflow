...
// Fetched step-by-step (properties -> units -> leases -> invoices) rather
// than one deep nested query, for the same reliability reason noted back
// in Step 10 — deep PostgREST embedded filters can be fragile.
async function getLandlordScopeIds(landlordId: string) {
  ...
  const { data: units, error: unitsError } = propertyIds.length
    ...
    : { data: [] as { id: string; property_id: string; status: string }[], error: null };
